...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_list:
        ...
        vote_count = county_dict[county_name]

        # 6c: Calculate the percentage of votes for the county.
        vote_percentage = float(vote_count)/float(total_votes) *100

        # 6d: Print the county results to the terminal.
        print(f"{county_name}: {vote_percentage:.1f}% ({vote_count:,})")
        ...
        if county_dict[county_name] >= voter_turnout:
    ...
    for candidate_name in candidate_dict:
        ...
        votes = candidate_dict.get(candidate_name)
